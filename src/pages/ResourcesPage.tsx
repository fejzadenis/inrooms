...
export function ResourcesPage() {
  ...
  const categories = [
    { id: 'general', name: 'General', icon: HelpCircle, color: 'bg-blue-500/30 text-blue-400' },
    { id: 'account', name: 'Account', icon: Users, color: 'bg-green-500/30 text-green-400' },
    { id: 'billing', name: 'Billing', icon: CreditCard, color: 'bg-purple-500/30 text-purple-400' },
    { id: 'events', name: 'Events', icon: Calendar, color: 'bg-orange-500/30 text-orange-400' },
    { id: 'products', name: 'Products', icon: Rocket, color: 'bg-red-500/30 text-red-400' },
    { id: 'security', name: 'Security', icon: Shield, color: 'bg-indigo-500/30 text-indigo-400' },
  ];
    ...
    <MainLayout>
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <RevealOnScroll>
          ...
          >
            ...
            >
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 neon-blue">Frequently Asked Questions</h1>
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 text-lg border border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm bg-gray-800/50 text-white"
              />
            </div>
          </motion.div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          ...
          >
            <a href="mailto:support@inrooms.com" className="group glass-dark bg-gradient-to-br from-blue-500/20 to-blue-600/20 rounded-xl p-6 text-white hover:from-blue-600/30 hover:to-blue-700/30 transition-all duration-200 shadow-lg border border-blue-500/30 hoverable">
              <MessageSquare className="w-8 h-8 mb-4 group-hover:scale-110 transition-transform duration-200" />
              <h3 className="text-xl font-semibold mb-2">Contact Support</h3>
              <p className="text-blue-200">Get personalized help from our team</p>
            </a>
            <a href="/help" className="group glass-dark bg-gradient-to-br from-green-500/20 to-green-600/20 rounded-xl p-6 text-white hover:from-green-600/30 hover:to-green-700/30 transition-all duration-200 shadow-lg border border-green-500/30 hoverable">
              ...
              <p className="text-green-200">Browse our comprehensive help articles</p>
            </a>
            <a href="mailto:feedback@inrooms.com" className="group glass-dark bg-gradient-to-br from-purple-500/20 to-purple-600/20 rounded-xl p-6 text-white hover:from-purple-600/30 hover:to-purple-700/30 transition-all duration-200 shadow-lg border border-purple-500/30 hoverable">
              ...
              <p className="text-purple-200">Help us improve your experience</p>
            </a>
          </motion.div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          ...
          >
            <h2 className="text-2xl font-bold text-white mb-6 neon-blue">Browse by Category</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <button
                onClick={() => setActiveCategory(null)}
                className={`p-4 rounded-xl transition-all duration-200 hoverable ${
                  !activeCategory 
                    ? 'glass-dark bg-gradient-to-br from-indigo-500/30 to-indigo-600/30 text-white shadow-lg border border-indigo-500/30' 
                    : 'glass-dark border border-gray-700 hover:border-indigo-500/30 hover:bg-indigo-500/10'
                }`}
              >
                <div className="text-center">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center mx-auto mb-3 ${
                    !activeCategory ? 'bg-white bg-opacity-10' : 'bg-indigo-500/20'
                  }`}>
                    <Star className={`w-6 h-6 ${!activeCategory ? 'text-white' : 'text-indigo-400'}`} />
                  </div>
                  <h3 className={`font-medium ${!activeCategory ? 'text-white' : 'text-gray-300'}`}>All Topics</h3>
                  <p className={`text-sm mt-1 ${!activeCategory ? 'text-indigo-200' : 'text-gray-400'}`}>
                    {faqs.length} answers
                  </p>
                </div>
              </button>
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-xl transition-all duration-200 hoverable ${
                      isActive 
                        ? `glass-dark bg-gradient-to-br from-${category.color.split('-')[1]}-500/30 to-${category.color.split('-')[1]}-600/30 text-white shadow-lg border border-${category.color.split('-')[1]}-500/30` 
                        : 'glass-dark border border-gray-700 hover:border-indigo-500/30 hover:bg-indigo-500/10'
                    }`}
                  >
                    <div className="text-center">
                      <div className={`w-12 h-12 rounded-lg flex items-center justify-center mx-auto mb-3 ${
                        isActive ? 'bg-white bg-opacity-10' : category.color
                      }`}>
                        <Icon className={`w-6 h-6 ${isActive ? 'text-white' : 'text-white'}`} />
                      </div>
                      <h3 className={`font-medium ${isActive ? 'text-white' : 'text-gray-300'}`}>{category.name}</h3>
                      <p className={`text-sm mt-1 ${
                        isActive ? `text-${category.color.split('-')[1]}-200` : 'text-gray-400'
                      }`}>
        ...
        <RevealOnScroll>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white neon-blue">
                {activeCategory 
                  ? `${categories.find(c => c.id === activeCategory)?.name} Questions`
                  : 'Frequently Asked Questions'
                }
              </h2>
              <span className="text-sm text-gray-400 bg-gray-800/50 px-3 py-1 rounded-full glass-dark">
                {filteredFaqs.length} {filteredFaqs.length === 1 ? 'result' : 'results'}
              </span>
            </div>

            {filteredFaqs.length === 0 ? (
              <div className="text-center py-12 glass-dark rounded-xl border border-gray-700">
                ...
                <GlowButton onClick={() => { setSearchTerm(''); setActiveCategory(null); }}>
                  Clear Filters
                </GlowButton>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredFaqs.map((faq) => (
                  <motion.div 
                    ...
                    className="glass-dark rounded-xl border border-gray-700 overflow-hidden hover:border-indigo-500/30 transition-colors duration-200"
                  >
                    <button
                      onClick={() => toggleFaq(faq.id)}
                      className="w-full flex items-center justify-between p-6 cursor-pointer hover:bg-gray-800/30 transition-colors duration-200 text-left hoverable"
                    >
        ...
        <RevealOnScroll>
          <motion.div 
            ...
            className="relative overflow-hidden bg-gradient-to-r from-indigo-600/40 via-purple-600/40 to-blue-600/40 rounded-xl p-8 text-white shadow-xl border border-indigo-500/30 glass-dark"
          >
            ...
            <div className="relative z-10 text-center">
              <h3 className="text-2xl font-bold mb-4 neon-blue">Still need help?</h3>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="mailto:support@inrooms.com">
                  <GlowButton variant="outline" className="bg-white/10 text-white hover:bg-white/20 border-white/20 hoverable">
                    <Mail className="w-4 h-4 mr-2" />
                    Email Support
                  </GlowButton>
                </a>
                <GlowButton className="bg-indigo-600/80 hover:bg-indigo-700/80 text-white border border-indigo-500/50 hoverable">
