/*
# Fix Custom Quotes and Product Feedback
...
2. Changes
   ...
   - Add default_payment_method to stripe_customers
   - Create email verification timestamp function and trigger
...
BEGIN
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Admins can read all custom quotes"
      ON custom_quotes
      FOR SELECT
      TO authenticated
      USING (
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can create custom quotes"
      ON custom_quotes
      FOR INSERT
      TO authenticated
      WITH CHECK (true)';
...
BEGIN
  ...
  IF NOT function_exists THEN
    EXECUTE 'CREATE FUNCTION update_email_verified_timestamp()
    RETURNS TRIGGER AS $$
    BEGIN
...
BEGIN
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Anyone can view public feedback"
      ON product_feedback
      FOR SELECT
      TO authenticated
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can manage their own feedback"
      ON product_feedback
      FOR ALL
      TO authenticated
...
BEGIN
  ...
  ) INTO column_type_matches;
  
  IF constraint_exists AND column_type_matches THEN
