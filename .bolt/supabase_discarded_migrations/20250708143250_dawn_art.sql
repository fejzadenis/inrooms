/*
  # Custom Quotes and Product Feedback Tables

  1. New Tables
    - `custom_quotes` - For storing enterprise quote requests
  ...
  2. Changes
    ...
    - Fix foreign key constraints for Stripe tables
...
CREATE POLICY IF NOT EXISTS "Admins can read all custom quotes"
  ON custom_quotes
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS "Users can create custom quotes"
  ON custom_quotes
  FOR INSERT
  TO authenticated
  WITH CHECK (true);
...
CREATE POLICY IF NOT EXISTS "Anyone can view public feedback"
  ON product_feedback
  FOR SELECT
  TO authenticated
