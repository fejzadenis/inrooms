/*
  # Update Subscription Triggers and Fix User Updates

  1. New Functions
    - `update_user_subscription_from_stripe` - Updates user subscription data when Stripe subscriptions change
    - `handle_subscription_cancellation` - Handles subscription cancellations
    - `update_user_from_checkout_session` - Updates user data when checkout sessions complete
    - `update_user_from_customer` - Updates user data when customers are created

  2. Triggers
    - Add triggers to automatically update user records when related Stripe data changes
    - Ensure proper cascading of subscription status changes
