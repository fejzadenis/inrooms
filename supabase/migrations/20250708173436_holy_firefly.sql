/*
  # Add Subscription Update Triggers

  1. New Functions
    - `update_user_subscription_from_stripe` - Updates user subscription data when a Stripe subscription is updated
    - `update_user_from_checkout_session` - Updates user data when a checkout session is completed
    - `handle_subscription_cancellation` - Handles subscription cancellation
    - `update_user_from_customer` - Updates user data when a customer is created

  2. Triggers
    - Add triggers to automatically update user data when Stripe events occur
    - Ensure proper cascade of subscription data to user records
