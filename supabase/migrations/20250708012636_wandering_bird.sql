/*
  ...
  1. New Tables
    - `custom_quotes` - For storing custom enterprise quote requests
    - `product_feedback` - For storing user feedback on products
  
  2. Changes
    - Add metadata fields to stripe_subscriptions and stripe_customers
...
DO $$
BEGIN
  -- First check if the constraints exist and have the wrong type
  IF EXISTS (
