/*
  # Fix user_id column types for Firebase compatibility

  1. Changes
    - Change user_id columns from uuid to text in registrations table
    - Change user_id columns from uuid to text in other related tables
    - Update foreign key constraints to match new text type
    - Ensure all user_id references are consistent with Firebase string IDs

  2. Security
    - Maintain existing RLS policies
    - Update any policies that reference user_id columns
*/
...
-- Create an RPC function to get user subscription data (if it doesn't exist)
