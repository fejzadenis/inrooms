/*
  # Create Subscription Update Triggers

  1. New Functions
    - `update_user_subscription_from_stripe` - Updates user data when subscriptions change
    - `update_user_from_checkout_session` - Updates user data when checkout completes
    - `handle_subscription_cancellation` - Handles subscription cancellations
    - `update_user_from_customer` - Updates user data when customer is created

  2. Triggers
    - Automatically update user data when Stripe records change
    - Ensure subscription status, quota, and customer ID are always in sync
